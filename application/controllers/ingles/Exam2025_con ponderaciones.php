...
$pass     = $this->input->post('pass');
    ...
    foreach ($resultados_crudos as $fila) {
       ...
       $resultados_por_alumno[$nombre]['rubros'] = [
    'grammar' => [
        'correctas' => $fila['total_grammar'],
        'total'     => $fila['total_grammar_examen']
    ],
    'reading' => [
        'correctas' => $fila['total_reading'],
        'total'     => $fila['total_reading_examen']
    ],
    'vocabulary' => [
        'correctas' => $fila['total_vocabulary'],
        'total'     => $fila['total_vocabulary_examen']
    ]
];
    ...
    foreach ($resultados_por_alumno as $nombre_alumno => $datos) {
        ...
        $pdf->SetFont('gothamblack', '', 16);
        $pdf->SetTextColor(0,0,0);
        $pdf->Cell(40, 10, $valorObtenido." ".$datos['nivel'], 0, 1, 'C');
...
foreach ($rubros as $nombreRubro => $valorRubro) {
    ...
    $puntajeRubro = ($valorRubro['total'] > 0)
        ? ($valorRubro['correctas'] / $valorRubro['total']) * 10
        : 0;

    $this->bloqueModuloConRegleta1(
        $pdf,
        $nombreRubro,
        number_format($puntajeRubro, 2),
        [
            'correctas' => $valorRubro['correctas'],
            'total'     => $valorRubro['total']
        ],
        $vinos
    );
}
...
<table border="1" cellpadding="6">
    ...
    <tr>
        <td>Grammar</td>
        <td>'.$datos['rubros']['grammar']['correctas'].' correctas de '.$datos['rubros']['grammar']['total'].'</td>
    </tr>
    <tr>
        <td>Reading</td>
        <td>'.$datos['rubros']['reading']['correctas'].' correctas de '.$datos['rubros']['reading']['total'].'</td>
    </tr>
    <tr>
        <td>Vocabulary</td>
        <td>'.$datos['rubros']['vocabulary']['correctas'].' correctas de '.$datos['rubros']['vocabulary']['total'].'</td>
    </tr>
</table>
';

$pdf->writeHTML($htmlResumen, true, false, false, '');
...
private function bloqueModuloConRegleta1(
    $pdf,
    $nombreRubro,
    $puntajeFormateado,
    $datos,
    ...
    $pdf->Cell(
        0,
        5,
        $datos['correctas'].' correctas de '.$datos['total'].' reactivos',
        0,
        1
    );
    ...
    $pdf->Rect($inicioX, $pdf->GetY(), $anchoBarra, $altoBarra);

    // Escala 0–10
    for ($i = 0; $i <= 10; $i++) {
        $x = $inicioX + ($anchoBarra / 10) * $i;
        $pdf->Line($x, $pdf->GetY(), $x, $pdf->GetY() + $altoBarra);
    }

    // Indicador
    $puntaje = floatval($puntajeFormateado);
    $posicion = $inicioX + ($puntaje / 10) * $anchoBarra;
    ...
    $pdf->Ln(10);
    $pdf->SetFont('gothamblack', '', 10);
    $pdf->Cell(0, 5, 'Puntaje: '.$puntajeFormateado, 0, 1);
