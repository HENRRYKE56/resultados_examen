...
class Roles extends BaseController
{
    ...
    public function addModule() {
        ...
        $this->global['pageTitle'] = 'Nuevos Modulos';
    ...
    function edit($roleId = NULL)
    {
        ...
        else
        {
            ...
            $data['roleAccessMatrix'] = json_decode($roleAccessMatrix->access);
             $data['moduleList'] = json_decode($roleAccessMatrix->access);

      //       $data['moduleList'] = $this->config->item('moduleList');
            
          
            $this->global['pageTitle'] = 'SEIEM : Editar Rolees';
    ...
    public function storeAccessMatrix()
    {
        ...
        $resultado = $this->rm->getRoleAccessMatrix($roleId);
       
       
         $roleAccessMatrix=$resultado->access;  
        $data['roleAccessMatrix'] = json_decode($roleAccessMatrix);
       //  $data['moduleList'] = json_decode($roleAccessMatrix->module);

// Recoge los datos enviados por POST
$roleAccessMatrix = $_POST['access'];

// Arreglo para almacenar el resultado
$structuredAccess = [];

// Recorrer cada módulo dentro de access
foreach ($roleAccessMatrix as $module => $permissions) {
    $structuredAccess[]= [
        'module' => $module,
        'total_access' => isset($permissions['total_access']) && $permissions['total_access'] === 'on' ? 1 : 0,
        'list' => isset($permissions['list']) && $permissions['list'] === 'on' ? 1 : 0,
        'create_records' => isset($permissions['create_records']) && $permissions['create_records'] === 'on' ? 1 : 0,
        'edit_records' => isset($permissions['edit_records']) && $permissions['edit_records'] === 'on' ? 1 : 0,
        'delete_records' => isset($permissions['delete_records']) && $permissions['delete_records'] === 'on' ? 1 : 0,
...
$jsonResult = json_encode($structuredAccess, JSON_PRETTY_PRINT);

$accessMatrix = ["access"=>$jsonResult, 'updatedBy'=>$this->vendorId, 'updatedDtm'=>date('Y-m-d H:i:s')];
