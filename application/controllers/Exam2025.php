...
foreach ($resultados_por_alumno as $nombre_alumno_reporte => $datos_alumno) {
    ...
    // Línea superior color vino
    $pdf->SetDrawColor($vino[0], $vino[1], $vino[2]);
    $pdf->SetLineWidth(1.5);
    $pdf->Line(20, 30, 200, 30);

    // --------------------------------------------------------------
    // CÁLCULO DEL PUNTAJE OBTENIDO (CORREGIDO Y SIN REDUNDANCIA)
    // --------------------------------------------------------------
    $valorObtenido = 0;
    ...
    // Calculamos el promedio dividiendo entre los 4 rubros
    $valorObtenido = number_format(($valorObtenido / 4), 2);
    ...
    // ... (El resto de tu código para la barra de puntaje va aquí y funciona igual) ...
    
    // --- CÓDIGO PARA LOS RUBROS (no necesita cambios) ---
    $pdf->Ln(8);
    $listaRubros = [
   ...
   public function sede()
    {
        ...
        else
        {
            ...
            $this->global['pageTitle'] = 'SEIEM : Reporte de Resultados por Sede Examen 2025';

            $this->loadViews("Exam2025/sedes", $this->global, $this->data, NULL);
        }
    }
...
$nombre_archivo="";
    ...
    $todos_los_resultados_crudos = $this->em->get_all_resultados_for_sedes($ies, $sede);
...
$nombre_archivo="";
    ...
    // *** CAMBIO CLAVE #1: UNA SOLA CONSULTA A LA BD ***
    // Usamos nuestra nueva función optimizada
    $todos_los_resultados_crudos = $this->em->get_all_resultados_for_planes($ies, $sede);
//cho '<pre>'; print_r( $todos_los_resultados_crudos); echo '</pre>'; die();
    // *** CAMBIO CLAVE #2: AGRUPAR DATOS EN PHP (en lugar de en la BD) ***
    // Transformamos el array plano en un array anidado por alumno
    $resultados_por_alumno = [];
    foreach ($todos_los_resultados_crudos as $fila) {
        $nombre_alumno = $fila['programa'];
